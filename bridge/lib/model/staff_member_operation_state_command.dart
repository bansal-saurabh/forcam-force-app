...
class StaffMemberOperationStateCommand {
...
/* The type of the command */
  ...
  StaffMemberOperationStateCommand();

  @override
  String toString() {
    return 'StaffMemberOperationStateCommand[forcedLogOff=$forcedLogOff, source=$source, staffMemberId=$staffMemberId, staffMemberTimeAttendanceBreakId=$staffMemberTimeAttendanceBreakId, type=$type, operationId=$operationId, staffMemberStateId=$staffMemberStateId, workplaceId=$workplaceId, ]';
  }

  StaffMemberOperationStateCommand.fromJson(Map<String, dynamic> json) {
    if (json == null) return;
  ...
  static List<StaffMemberOperationStateCommand> listFromJson(List<dynamic> json) {
    return json == null ? new List<StaffMemberOperationStateCommand>() : json.map((value) => new StaffMemberOperationStateCommand.fromJson(value)).toList();
  }

  static Map<String, StaffMemberOperationStateCommand> mapFromJson(Map<String, Map<String, dynamic>> json) {
    var map = new Map<String, StaffMemberOperationStateCommand>();
    if (json != null && json.length > 0) {
      json.forEach((String key, Map<String, dynamic> value) => map[key] = new StaffMemberOperationStateCommand.fromJson(value));
    }
    return map;
  }
}
